...
public class Boss : MonoBehaviour
{
    ...
    public void CheckPuzzleBullet(string playerRecipe, int damage)
    {
        ...
        // 🌟 ถ้าบอสกำลังร่ายท่าไม้ตายอยู่ จะยกเลิกการเจาะเกราะทั้งหมด!
        if (isUninterruptible)
        {
            Debug.Log("🛡️ บอสร่ายเวทอยู่ โดนขัดไม่ได้!!");
            ShowDamagePopup(0, Color.gray, 3f); // เด้งเลข 0 สีเทาบอกผู้เล่นว่าตีไม่เข้า
            return; // เด้งออกไปเลย ไม่เช็กรหัสต่อ
        }
    ...
    public void TakeDamage(int damage)
    {
        ...
        if (isBroken)
        {
            ...
            if (currentHealth <= 0)
            {
                ...
                // 🌟 สำคัญมาก! หยุดการโจมตีทุกอย่างทันที (กันบั๊กเลเซอร์ค้างตอนบอสตาย)
                StopAllCoroutines();
